...
async function openCamera() {
  ...
  try {
    ...
    showToast('Câmera aberta. Encaixe as fileiras nas linhas.');
...
$('#fileInput').addEventListener('change', (event) => {
  ...
  $('#sourceLabel').textContent = 'FOTO'; showToast('Foto carregada. Agora ajuste as linhas.');
});
...
$('#resetGuides').addEventListener('click', () => { spacing.value = 62; opacity.value = 82; updateRange(spacing, $('#spacingValue')); updateRange(opacity, $('#opacityValue'), '%'); showToast('Guias restauradas.'); });
